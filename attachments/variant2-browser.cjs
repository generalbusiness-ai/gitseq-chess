...
(async()=>{
 ...
 const friction={white:{clicks:0,changes:0},black:{clicks:0,changes:0}};
 for (let i=0;i<contexts.length;i++) {const who=i===0?'white':'black';await contexts[i].exposeBinding('__fixtureInteraction',(_,kind)=>{friction[who][kind]++});await contexts[i].addInitScript(()=>{document.addEventListener('click',()=>window.__fixtureInteraction('clicks'),true);document.addEventListener('change',()=>window.__fixtureInteraction('changes'),true)})}
 const [white,black]=await Promise.all(contexts.map(c=>c.newPage()));
 async function clearBrowserStorage(){await contexts[0].clearCookies();await white.evaluate(async()=>{localStorage.clear();sessionStorage.clear();for(const db of await indexedDB.databases()){await new Promise((resolve,reject)=>{const r=indexedDB.deleteDatabase(db.name);r.onsuccess=resolve;r.onerror=()=>reject(r.error);r.onblocked=()=>reject(new Error('blocked fixture database removal'))})}if(localStorage.length||sessionStorage.length)throw new Error('storage clearing failed')});await white.reload();}
 let onboarding;
 ...
 try{
  ...
  await black.locator('#join-game').click();await black.locator('#action-confirmation').waitFor({state:'visible'});const joined=await submit(black);assert.equal(joined.effective,true);onboarding=JSON.parse(JSON.stringify(friction));
  ...
  await clearBrowserStorage();const unanchored=await key(white);assert.notEqual(unanchored,firstKey);await prepare(white,'g2g4');const noAnchor=await submit(white);assert.equal(noAnchor.effective,false);assert.match(noAnchor.reason,/seat|actor|identity|player/i);
  ...
  await clearBrowserStorage();const recoveredKey=await key(white);signer=root;await anchor(white);await move(white,'g2g4');await black.waitForFunction(()=>document.querySelector('#last-move').textContent.includes('g2g4'));
  ...
  const result={...meta,twoIsolatedBrowserContexts:true,platform:process.platform,chromium:await browser.version(),containerHostNetwork:true,firstKey,recoveredKey,oldKeyRecovery:true,tabReloadRecovery:true,explicitBrowserStorageCleared:true,storageClears:2,onboardingInteractions:onboarding,totalInteractions:friction,unanchoredRefused:noAnchor,wrongIdentityRefused:wrong,preprovisionedNegativeKeys:negativeResults,exactLostResponseRetry:{record:retried.record,depth:retried.depth},final:last,outcome:await white.locator('.outcome').innerText(),pageErrors:errors};fs.writeFileSync('/evidence/browser-summary.json',JSON.stringify(result,null,2));console.log(JSON.stringify(result));
