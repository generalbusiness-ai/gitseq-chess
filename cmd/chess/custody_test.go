...
func TestKeyReadableByAnyoneElseIsRefused(t *testing.T) {
	for _, mode := range []os.FileMode{0o644, 0o640, 0o604, 0o660} {
		...
		if _, err := ensureKey(managedStore(t, root)); err == nil || !strings.Contains(err.Error(), "only its owner") {
			t.Fatalf("mode %04o was accepted: %v", mode, err)
		}
	}
}

func TestOwnerOnlyKeyIsAccepted(t *testing.T) {
	root := t.TempDir()
	want := writeKey(t, filepath.Join(root, "chess", "player.key"), 0o600)
	got, err := ensureKey(managedStore(t, root))
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(got, want) {
		t.Fatal("ensureKey returned a different key than the file holds")
	}
}
...
func TestNamedKeyPathIsAllowedOutsideAnyRepository(t *testing.T) {
	...
	if _, err := ensureKey(namedStore(t, loose)); err == nil || !strings.Contains(err.Error(), "only its owner") {
